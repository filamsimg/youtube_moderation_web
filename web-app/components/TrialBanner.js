...
import { Sparkles, ShieldCheck, AlertCircle, ArrowRight, X } from 'lucide-react';
...
export default function TrialBanner({ className = '', dismissible = false }) {
  ...
  const handleDismissOffer = () => {
    ...
    toast.info('Penawaran trial premium disembunyikan dari halaman ini.');
  };

  const handleDismissExpired = () => {
    localStorage.setItem('athena_trial_expired_dismissed', 'true');
    setExpiredDismissed(true);
    toast.info('Pemberitahuan trial berakhir disembunyikan dari halaman ini.');
  };

  // Jika profil belum siap, atau user bukan FREE tier, tidak usah tampilkan banner
  if (!mounted || !profile || profile.tier !== 'FREE') return null;
